...
def lstm_prediction_dummy(input_signal):
    model = load_model("model500.h5")  # Pretend to load the model
...
def get_roi_avg(frame, view, face_points, draw_rect=True):
    ...
    if draw_rect:
        cv2.rectangle(view, (fh_left, fh_top), (fh_right,
                      fh_bottom), color=(0, 255, 0), thickness=2)
        cv2.rectangle(view, (nose_left, nose_top), (nose_right,
                      nose_bottom), color=(0, 255, 0), thickness=2)
