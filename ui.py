...
class mainwin(QMainWindow, Ui_MainWindow):
    ...
    def lstm_prediction_dummy(input_signal):
        model = load_model("modele500.h5")  # Pretend to load the model

        processed_signal = np.expand_dims(input_signal, axis=0)  # Pretend processing

        prediction = model.predict(processed_signal)  # Dummy prediction step
    ...
    def DisplaySignal(self):
        ...
        if self.processor.series_class.Flag_Queue:
            ...
            Label_Text = "Fs: \t\t"+str(self.processor.series_class.fps)+"\nFore BPM: \t"+str(
                self.bpm_fore)+"\nFore: "+str(self.confidence_fore*100)+"%\nLeft BPM: \t"+str(self.bpm_left)+"\nLeft: "+str(self.confidence_left*100)+"%\nRight BPM:\t"+str(self.bpm_right)+"\nRight:"+str(self.confidence_right*100)+"%\n\nBPM Overall: \t"+str(self.bpm_avg)
